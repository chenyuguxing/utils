...
public class AccountBean implements EntityBean {
	...
	public void ejbLoad() {
		...
		try {
			...
			throw new EJBException(
				"Account " + pk + " failed to load from database",
				ex);
		} finally { // 释放数据库连接
			try {
				if (pstmt != null)
	...
	public void ejbStore() {
		...
		try {
			...
			throw new EJBException(
				"Account " + accountID + " failed to save to database",
				ex);
		} finally { // 释放数据库连接
			try {
				if (pstmt != null)
	...
	public AccountPK ejbFindByPrimaryKey(AccountPK key)
		...
		try {
			...
			// 在数据库中查找这个实体
			pstmt =
				conn.prepareStatement("select id from accounts where id = ?");
	...
	public Connection getConnection() throws Exception {
		try {
			...
			javax.sql.DataSource ds =
				(javax.sql.DataSource) ctx.lookup("java:/MSSQLDS");
			//这里要注意一致
